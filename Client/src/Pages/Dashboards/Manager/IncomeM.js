...
export default function Income() {
          ...
          .then((res) => {
            if (
              ...
              const todayincome = res.data.filter(
                (element) => format(element.date, "yyyy-MM-dd") === format(currentDate, "yyyy-MM-dd")
              );
  ...
  /* get today card paymetns and cash payments */
  useEffect(()=>{
    let card=0;
    let cash=0;
    today.forEach((income)=>{
      if(income.paymentMethod=="Card"){
        card+=income.amount;
      }else{
        cash+=income.amount;
